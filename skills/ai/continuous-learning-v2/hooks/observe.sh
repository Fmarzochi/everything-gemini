...
#   - EGC observing its own Haiku observer sessions (self-loop)
...
case "${GEMINI_CODE_ENTRYPOINT:-cli}" in
  cli|sdk-ts|egc-desktop) ;;
  *) exit 0 ;;
esac

# Layer 2: minimal hook profile suppresses non-essential hooks.
# EGC_* is canonical; ECC_* is the legacy fallback (never removed).
[ "${EGC_HOOK_PROFILE:-${ECC_HOOK_PROFILE:-standard}}" = "minimal" ] && exit 0

# Layer 3: cooperative skip env var for automated sessions (canonical + legacy).
[ "${EGC_SKIP_OBSERVE:-${ECC_SKIP_OBSERVE:-0}}" = "1" ] && exit 0
...
# Layer 5: known observer-session path exclusions (canonical + legacy env).
_EGC_SKIP_PATHS="${EGC_OBSERVE_SKIP_PATHS:-${ECC_OBSERVE_SKIP_PATHS:-observer-sessions,.gemini-mem}}"
if [ -n "$STDIN_CWD" ]; then
