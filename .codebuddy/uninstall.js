...
async function doUninstall() {
  ...
  console.log('EGC CodeBuddy Uninstaller');
  console.log('==========================');
  console.log('');
  ...
  if (!fs.existsSync(manifest)) {
    ...
    console.log('  1. EGC was installed with an older version without manifest support');
  ...
  console.log('Found manifest file - will only remove files installed by EGC');
  console.log('');

  const confirmed = await promptConfirm(`Are you sure you want to uninstall EGC from ${codebuddyDirName}? (y/N) `);
  if (!confirmed) {
    console.log('Uninstall cancelled.');
    process.exit(0);
  }
