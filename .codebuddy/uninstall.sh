...
do_uninstall() {
    ...
    echo "EGC CodeBuddy Uninstaller"
    echo "=========================="
    echo ""
    ...
    if [ ! -f "$MANIFEST" ]; then
        echo "Warning: No manifest file found (.egc-manifest)"
        echo ""
        echo "This could mean:"
        echo "  1. EGC was installed with an older version without manifest support"
    ...
    echo "Found manifest file - will only remove files installed by EGC"
    echo ""
    read -p "Are you sure you want to uninstall EGC from $CODEBUDDY_DIR? (y/N) " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        echo "Uninstall cancelled."
        exit 0
